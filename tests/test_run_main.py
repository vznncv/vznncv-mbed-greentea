...
import pytest
from hamcrest import assert_that, equal_to, not_, string_contains_in_order
...
def _build_dummy_main_binaries(program_config: ProgramConfig):
    toolchain = program_config.resolve_toolchaing()
    profile = program_config.resolve_profile()
    build_dir = os.path.join(
        program_config.get_build_dir(),
        _TEST_TARGET_NAME.upper(),
        f'{toolchain.upper()}-{profile.upper()}'
    )
    os.makedirs(build_dir, exist_ok=True)
    artifact_name = program_config.program_name

    for ext in _BUILD_EXTS:
        artifact_path = os.path.join(build_dir, f'{artifact_name}.{ext}')
        with open(artifact_path, 'wb') as f:
            f.write(b"-- dummy data --")


def build_run_main_dummy_mbed_cli_callback(project_dir, *, baudrate) -> Callable:
    def mbed_cli_callback(cmd_name, cmd_args, *, cwd):
        ...
        if '--config' in cmd_args:
            ...
            _build_dummy_main_binaries(ProgramConfig(project_dir))
            return '', 'success', 0
...
def test_run_successful_test(mbed_project, mbed_cli_mock):
    ...
    with VirtualComPortStub(
        ...
        result = runner.invoke(
            main_cli, ['run-main', '--board-manager-script', 'target_manager.sh'],
